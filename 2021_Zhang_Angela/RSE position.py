...
'''
Produces the attitude, position and velocity root-square-error plots and saves them as figure
'''
...
for i in range(len(datatype)):
    for k in output.keys():        
        if datatype[i] in k:
            ...
            if datatype[i] == "Yaw":
                y = [wrap_360(v[1]) for v in output[k]] #EK3 Yaw is -180 to 180, wrap around so between 0 and 360 degrees
            else:
...
#Get EKF and EqF roll pitch yaw to match time with Simulator (SIM) time
...
#Compute the angle of relative rotation between true and estimated attitude for EKF
...
if "EqF1" in args.log:
    start_t = 75
    end_t = 175
elif "bowtie" in args.log:
    start_t = 120
    end_t = 260
elif "line1" in args.log:
    start_t = 100
    end_t = 250
...
##Compute position RSE based on relative position
...
plt.figure() #Plot position RSE using GPS diff function
...
ax16[0].set_xlim([xlim_left,xlim_right])
...
ax16[1].set_xlim([xlim_left,xlim_right])
...
ax16[2].set_xlim([xlim_left,xlim_right])
...
ax17[0].set_xlim([xlim_left,xlim_right])
...
ax17[1].set_xlim([xlim_left, xlim_right])
...
ax17[2].set_xlim([xlim_left, xlim_right])
figure = plt.gcf()  # get current figure
figure.set_size_inches(8, 5) # set figure's size manually to your full screen
