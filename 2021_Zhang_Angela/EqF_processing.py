...
parser.add_argument('--addnoise', type= str, default = 'false', help='type true to manually add Gaussian noise')
...
#Add Gaussian noise according to the EKF parameters, else manually change the code in the else statement
...
class EqF:
    def __init__(self,gravity, param):
            ...
            if param == 'false':
                ...
                self.Q_baro = np.square(par_dict.get('EK3_ALT_M_NSE'))
...
def estimate(filename, param):
    ...
    if addnoise == 'true':
        outputfile = 'EqFoutput_' + filename + '_addnoise_EKparam ' + param + '.pckl'
    else:     
